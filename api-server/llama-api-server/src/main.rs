...
async fn main() -> Result<(), ServerError> {
    ...
    // temperature
    let temp = matches.get_one::<f64>("temp").unwrap();
    ...
    // repeat penalty
    let repeat_penalty = matches.get_one::<f64>("repeat_penalty").unwrap();
    println!("[INFO] Repeat penalty: {penalty}", penalty = repeat_penalty);
